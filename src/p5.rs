use std::{fs::read_to_string, cmp::{min, max}};
...
fn aply_map_interval((int_l,int_r):Interval,map : & [Range]) -> Vec<Interval> {
    ...
    let middle = if middle_l <= middle_r {
        Some((middle_l + range.out_start - range.in_start
    ...
    let mut ret = match (lefts,rights) {
        (Some(mut l),Some(mut r)) => {l.append(&mut r);l},
        (None,Some(r)) => r,
        (Some(l),None) => l,
        (None,None) => vec![],
    };
        ...
        for range_raw in map {
            match range_raw[..] {
                [out_start,in_start,size] => {
                    maps.last_mut()
                        .expect("empty")
                        .push(Range{in_start,out_start,size});
                }
                _ => {panic!("bad range");},
            }
        }
